...
public class ItemProfileMember extends Item {
				...
				try {
						...
						// already exist, so what is the bonitaId ? 
							...
							if ( profileMember!=null)
									...
									organizationLog.log(true, "ItemProfileMember.saveInServer","Can't find the profileMember for ["+itemInformation.get(cstProfileName)+"] userName[" + itemInformation.get(cstUserName) + "] Group[" + itemInformation.get(cstGroupPath) + "] Role[" + itemInformation.get(cstRoleName)
													+ "] when its exist because the creation failed");
									

				} catch (CreationException e) {
						organizationLog.log(true, "ItemProfileMember.saveInServer","Cant' create ProfileMember on profile["+itemInformation.get(cstProfileName)+") userName[" + itemInformation.get(cstUserName) + "] Group[" + itemInformation.get(cstGroupPath) + "] Role[" + itemInformation.get(cstRoleName)
										+ "] Error " + e.toString());
				}
				return;
		}
		...
		protected static void photoAll(Item.StatisticOnItem statisticOnProfileMember, ParametersOperation parametersLoad,  ProfileAPI profileAPI, OrganizationLog organizationLog) {
				...
				try {
						...
						{
								// in that case, we MUST not set in the photo the profileMember from the User profile. Doing that, theses profileMember is never purge
		...
		protected static void purgeFromList(Item.StatisticOnItem statisticOnItemProfileMember,ParametersOperation parametersLoad, ProfileAPI profileAPI, OrganizationLog organizationLog) {
				...
				for (Long profileMemberId : statisticOnItemProfileMember.listKeyItem)
				{
						try {
								...
								// if the profile is part of a delete profile, it's normal that the profilemember is not found organisationLog.log(true, "ItemProfile.purgeFromList", "profileMember ["+profileMemberId+"] " + e.toString());
