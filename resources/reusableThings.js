...
(function() {
	var appCommand2 = angular.module('reusableThings', []);

//	Constant used to specify resource base path (facilitates integration into a Bonita custom page)
	appCommand2.constant('RESOURCE_PATH', 'pageResource?page=custompage_american&location=');

	appCommand2.directive('fileDropzone', function() {
		return {
			require: '^?form',
			restrict: 'A',
			scope: {
	...
	appCommand2.controller('TestCtrl', function($scope) {
		$scope.image = null
		$scope.imageFileName = ''
	});
