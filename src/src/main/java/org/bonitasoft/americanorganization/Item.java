...
public abstract class Item {
	...
	/**
	 * the source can give some contextual information on the source (for
	 * example, for a CSV loader, it maybe the line number. If the contextual
	 * information is set, then the log display it
	 */
	...
	/**
	 * according the itemName, return the correct class. Example : it itemName
	 * is GROUP, then return a ItemGroup
	 *
	 * @param itemName
	 * @return
	 */
	...
	public String setAttributes(final Map<String, String> value, final OrganizationLog organizationLog) {
		...
		for (final String key : itemInformation.keySet()) {
			if (key.trim().length() == 0) {
				// ok, the guy put some ;; at the end of the line, be tolerant
				continue;
			}
