...
/**
 * describe a new Source to load organization.
 * The source will be call first on a initInput, then by multiple getNextItem until there are no more item to return. Then a endInput is called.
 *
 */
public interface OrganizationIntSource {
		...
		/**
		 * get the next Item form the source, or null if there are no more item available in the source
		 * An Item is different object : ItelmUser, ItemGroup, ItemRole, ItemMemberShip, ItemProfile.
		 * Each Item 
		 * @param identityAPI 
