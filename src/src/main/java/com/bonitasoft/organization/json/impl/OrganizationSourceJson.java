package com.bonitasoft.organization.json.impl;
...
import com.bonitasoft.organization.ItemGroup;
import com.bonitasoft.organization.ItemMembership;
import com.bonitasoft.organization.ItemProfile;
import com.bonitasoft.organization.ItemProfileMember;
import com.bonitasoft.organization.ItemRole;
import com.bonitasoft.organization.ItemUser;
...
public class OrganizationSourceJson implements OrganizationIntSource {
		 ...
		 * @param login
		 * @param password
		 * @param urlLoginPostParameters if set, then the request is switch to POST
		 */
						...
						if (jsonList instanceof List) {
								...
								for (final Object jsonLine : listItem) {
										if (jsonLine instanceof Map) {
												...
												organizationLog.log(true, "com.twosigma.bonitasoft.organization.json.impl.OrganizationSourceJson", "Item[" + currentItem + "]:We expect a Map on each line : found "
																+ (jsonLine == null ? "null" : jsonLine.getClass().getName()));
										}
								}

						} else {
								organizationLog.log(true, "com.twosigma.bonitasoft.organization.json.impl.OrganizationSourceJson", "Item[" + currentItem + "]:We expect a List of Map "
												+ (jsonList == null ? "null" : jsonList.getClass().getName()));
						}
		...
		private boolean connectionLoginPassword(final OrganizationLog organizationLog) {
				...
				try {
						...
						{
								String cookieName = oneCookie.substring(0, oneCookie.indexOf("="));
								String cookieValue = oneCookie.substring(oneCookie.indexOf("=") + 1, oneCookie.length());
